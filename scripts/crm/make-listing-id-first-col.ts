...
async function arrangeView(viewId: string, objName: string, columns: string[]) {
  console.log(`\n--- Arranging View ${viewId} (${objName}) with ${columns.length} columns (First: ${columns[0]}) ---`);
  ...
  for (let i = 0; i < columns.length; i++) {
    ...
    const size = col === 'name' ? 440 : (col === 'listingId' ? 140 : 180);
...
async function main() {
  console.log('=== MAKING LISTING ID THE FIRST COLUMN (POSITION 0) ACROSS ALL VIEWS ===');
  ...
  console.log('\n✅ Listing ID is now the FIRST column (position 0) across all views!');
}
