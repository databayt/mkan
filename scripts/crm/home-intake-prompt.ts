...
export function buildIntakePrompt(input: PromptInput): string {
  ...
  lines.push(
    ...
    '- Area applies to all units unless a unit names its own; zone must be one of the ZONES values, city one of the CITIES values — otherwise null. Every other place word — a building or operator name (مشغل دبي), a market (سوق سكة حديد), a landmark (قريب من المطار), a street — goes to area.addressText, never to leftover.',
