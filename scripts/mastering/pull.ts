...
async function main(): Promise<void> {
  ...
  if (APPLY && runsMade.length && slackReady()) {
    ...
    const digest = [
      ...
      ...lines,
      `*Do:* drag them from \`~/mkan/inbox/originals/\` into the ChatGPT *Makan Mastering* project (standing prompt — nothing to paste), then save each render into \`~/mkan/inbox/\` keeping the \`<run-id> …\` prefix. The relay does the rest.`,
