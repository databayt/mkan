...
export function isInSudan(lat: number, lng: number): boolean {
  ...
  for (let i = 0, j = SUDAN_BORDER.length - 1; i < SUDAN_BORDER.length; j = i++) {
    const vi = SUDAN_BORDER[i];
    const vj = SUDAN_BORDER[j];
    if (!vi || !vj) continue;
    const [latI, lngI] = vi;
    const [latJ, lngJ] = vj;
...
export function kmToBorder(lat: number, lng: number): number {
  let best = Infinity;
  for (let i = 0, j = SUDAN_BORDER.length - 1; i < SUDAN_BORDER.length; j = i++) {
    const va = SUDAN_BORDER[j];
    const vb = SUDAN_BORDER[i];
    if (!va || !vb) continue;
    const [aLat, aLng] = va;
    const [bLat, bLng] = vb;
 ...
 *
 * The explicit undefined guards are for `noUncheckedIndexedAccess`, which this
 * file only started being held to when it moved from `scripts/` (excluded from
 * the typecheck) into `src/`. Every polyline here is a non-empty module
 * constant, so the guards never fire at runtime — they document the precondition
...
export function classifyPoint(lat: number | null | undefined, lng: number | null | undefined): PlaceHit {
  ...
  // STATES is a non-empty module constant; the fallback satisfies the compiler.
  let bestState: SudanState | undefined = STATES[0];
  ...
  return { verdict, city: 'OTHER', state: bestState?.code ?? 'UNKNOWN', km: null, kmToBorder: border };
}
