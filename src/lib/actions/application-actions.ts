...
export async function updateApplicationStatus(
  ...
  try {
    ...
    if (!canOverride(session, application.listing.hostId)) {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (parsed.data.status === ApplicationStatus.Approved) {
        ...
        // Generate the first-month Payment row so the tenant sees an
        // upcoming charge immediately. Subsequent months are generated by
        // the monthly cron (`generateMonthlyPayments`).
        const firstDueDate = new Date(leaseStartDate);
