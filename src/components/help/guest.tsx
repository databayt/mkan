  ...
  return (
    <div className="py-10">
      ...
      <div className="space-y-6">
        <h4 className="">{t?.title ?? "Recommended for you"}</h4>
        ...
        <div className="flex space-x-4  ">
          ...
          <Card className="border border-gray-300 max-w-xs pb-0 rounded-md">
            ...
            <CardContent className="px-4">
              <h5 className="">{t?.verification?.title ?? "Your identity is not fully verified"}</h5>
              <p className="text-sm leading-relaxed font-normal">
                {t?.verification?.description ?? "Identity verification helps us check that you're really you. It's one of the ways we keep Mkan secure."}
              </p>
            </CardContent>
            <div className="space-y-0">
              <div className="border-t border-gray-200 "></div>
              <Button variant="ghost" className="w-full justify-between text-start h-12 hover:bg-gray-50 border-0">
                <span className="font-semibold text-sm text-gray-900">{t?.verification?.checkStatus ?? "Check identity verification status"}</span>
                <ChevronRight className="w-4 h-4 text-gray-900 rtl:rotate-180" />
              </Button>
              <div className="border-t border-gray-200"></div>
              <Button variant="ghost" className="w-full justify-between text-start h-12 hover:bg-gray-50 border-0">
                <span className="font-semibold text-sm text-gray-900">{t?.verification?.learnMore ?? learnMore}</span>
                <ChevronRight className="w-4 h-4 text-gray-900 rtl:rotate-180" />
              </Button>
            </div>
          </Card>
          ...
          <Card className="border border-gray-300 max-w-xs pb-0 relative rounded-md">
            <CardHeader className="px-4">
              <div className="text-gray-500 text-xs font-semibold uppercase tracking-wide">{t?.quickLink ?? "QUICK LINK"}</div>
            </CardHeader>
            <CardContent className="px-4">
              <h5 className="">{t?.reservationDetails?.title ?? "Finding reservation details"}</h5>
              <p className="text-sm leading-relaxed font-normal">
