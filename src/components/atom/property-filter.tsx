...
const AirbnbFilter: React.FC<AirbnbFilterProps> = ({
  ...
  const PROPERTY_TYPES = [
    { id: 'house', label: t?.types?.house ?? 'House', icon: Home },
    { id: 'apartment', label: t?.types?.apartment ?? 'Apartment', icon: Building },
    { id: 'cabin', label: t?.types?.cabin ?? 'Cabin', icon: TreePine },
    { id: 'villa', label: t?.types?.villa ?? 'Villa', icon: Mountain },
  ];

  const AMENITIES = [
    { id: 'wifi', label: t?.amenitiesList?.wifi ?? 'Wifi', icon: Wifi },
    { id: 'parking', label: t?.amenitiesList?.freeParking ?? 'Free parking', icon: Car },
    { id: 'pool', label: t?.amenitiesList?.pool ?? 'Pool', icon: Waves },
    { id: 'kitchen', label: t?.amenitiesList?.kitchen ?? 'Kitchen', icon: UtensilsCrossed },
  ];
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          ...
          >
            ...
            <span className="font-medium">{t?.filtersButton ?? "Filters"}</span>
            {activeCount > 0 && (
              <Badge
                variant="default"
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold">{t?.filtersTitle ?? "Filters"}</DialogTitle>
          </DialogHeader>

          <div className="space-y-6 py-4">
            {/* Price Range */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium">{t?.priceRange ?? "Price range"}</h3>
              <div className="space-y-4">
                <div className="px-4">
                  <Slider
            ...
            {/* Property Type */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium">{t?.propertyType ?? "Property type"}</h3>
            ...
            {/* Amenities */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium">{t?.amenities ?? "Amenities"}</h3>
            ...
            {/* Booking Options */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium">{t?.bookingOptions ?? "Booking options"}</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-700">{t?.instantBook ?? "Instant Book"}</p>
                    <p className="text-xs text-gray-500">{t?.instantBookDesc ?? "Book without waiting for host approval"}</p>
                  </div>
                  <Checkbox
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-700">{t?.superhost ?? "Superhost"}</p>
                    <p className="text-xs text-gray-500">{t?.superhostDesc ?? "Stay with recognized hosts"}</p>
                  </div>
                  <Checkbox
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-700">{t?.guestFavorites ?? "Guest favorites"}</p>
                    <p className="text-xs text-gray-500">{t?.guestFavoritesDesc ?? "The most loved homes on Mkan"}</p>
                  </div>
                  <Checkbox
