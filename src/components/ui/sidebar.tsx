      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{dict?.common?.a11y?.sidebar ?? "Sidebar"}</SheetTitle>
            <SheetDescription>{dict?.common?.a11y?.sidebarDescription ?? "Displays the mobile sidebar."}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className="sr-only">{dict?.common?.a11y?.toggleSidebar ?? "Toggle Sidebar"}</span>
    </Button>
  )
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      aria-label={dict?.common?.a11y?.toggleSidebar ?? "Toggle Sidebar"}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={dict?.common?.a11y?.toggleSidebar ?? "Toggle Sidebar"}
      className={cn(
...
function SidebarMenuSkeleton({
  ...
  // Random width between 50 to 90%. Computed once via lazy useState so the
  // value is stable across renders and Math.random() doesn't run in render.
